...
- **Solar**: 6 V / 4.5 W panel (~750 mA peak at Vmp). **Measured Voc 7.4 V at ~25 °C** — Vmp under load sits in the 5.5–6.3 V range as expected, but the open-circuit reading exceeds the FireBeetle's nominal 4.5–6 V VIN spec. Cold-morning Voc could climb another ~5 % to ~7.8 V (Voc temperature coefficient ≈ −0.3 %/°C). This is outside DFRobot's recommended VIN range but well inside the onboard MPPT IC's absolute-max input (CN3791-class chips tolerate >25 V). Bench-test with the panel before deployment to confirm the FireBeetle behaves cleanly at full charge in bright sun.
- **Schottky**: in series with the panel's positive lead for reverse-current / reverse-polarity protection. **1N5822** (40 V / 3 A) is the chosen part — running well below its 3 A rating gives a lower forward drop (~0.3 V at our 750 mA peak) than the 1N5817 the original plan called for. 1N5819 also works (slightly higher V_F, ~0.5 V).
- **VIN sense (Stage 5.5)**: a **5:1 voltage divider — 39 kΩ (high) + 10 kΩ (low)** — from `VIN` (after the Schottky) to `GPIO1`, with the divider's mid-point feeding the ADC. Maps a worst-case 7.8 V to ~1.59 V at the ADC, leaving 1.7 V of headroom under the C6's 3.3 V ceiling. Used to detect charging by comparing VIN vs battery voltage.
...
6. **Stage 5 — Solar input + Schottky diode.** Hardware-only: **1N5822** (40 V / 3 A Schottky) in series between panel positive and FireBeetle `VIN`. Cathode (banded end) → `VIN`, anode → panel (+). Panel (−) straight to GND. The 3 A part runs well below rating at our ~750 mA peak, giving lower V_F than a 1N5817 would. 1N5819 is a viable substitute (~0.1 V more drop). Confirm via the battery voltage sensor that charging happens during sun.
7. **Stage 5.5 — Solar charging detection.** Hardware: **5:1 voltage divider (39 kΩ + 10 kΩ)** from VIN (post-Schottky) to **`GPIO1`** ADC. High-side resistor between VIN and the ADC pin; low-side resistor between the ADC pin and GND; ADC pin taps the junction. Optional 100 nF ceramic from ADC pin to GND for noise filtering. Firmware: `adc` sensor on `GPIO1` with `attenuation: 12db` and lambda multiplier `4.9`, exposing `Solar Voltage` (V). A binary template `Charging` derives state by comparing `Solar Voltage` to `Battery Voltage` — when VIN exceeds battery by a margin (suggest ~0.3 V), the panel is supplying current; when VIN sits at or below battery voltage, it isn't. Optional categorical text sensor (`Charging` / `Not Charging` / `Full`) layered on top of the binary plus battery percentage. Verify in HA across a sun-up/sun-down cycle: `Solar Voltage` should rise to ~5–6 V mid-day and drop to ~0 V at night, with `Charging` flipping accordingly. Why this stage exists at "5.5" rather than just absorbed into Stage 5: the Schottky install is a quick hardware-only step that can ship before a panel is even mounted; the divider + firmware adds a meaningful new entity with its own verification gate.
8. **Stage 6 — Mechanical assembly.** 3D-printed inner enclosure inside outer rain gauge body, Stevenson screen, Flexi Dip waterproofing, cable runs with drip loop.
9. **Stage 7 — Field deployment + calibration.** Side-by-side BME280 calibration with a reference for 1–2 days. Tipping bucket calibration: pour known volume, count tips, compute mm-per-tip. Mounting: 30 cm rim height for the rain gauge, 1.5 m on grass for the Stevenson screen.
10. **Stage 8 — Lightning detector (post-deployment).** Adds the AS3935 on the always-on 3V3 rail (NOT 3V3_C), wires the IRQ to a free LP-IO pin as a third deep-sleep wake source alongside timer + reed. ESPHome's `as3935_i2c` component handles the chip protocol; LCO antenna calibration runs once per install. Wake-cause branching extends to distinguish lightning from reed. Mounting in a separate enclosure on the same pole, away from the C6's WiFi radio. HA automation fires push notification when last strike distance < 20 km. See "Future Expansion" section above for the architectural deltas, mounting constraints, false-positive risks (load-shedding transients, neighbour pool pumps), and budget for an evening of disturber-rejection tuning post-install.
11. **Stage 9 — Wind speed + direction (post-deployment).** Adds a 3-cup anemometer on a roof-mounted mast and an AS5600 magnetic-encoder vane above it. Wind speed via `pulse_counter` sampling within the existing 10 s awake window — NOT a deep-sleep wake source (cups at 50 Hz would torch the power budget). Wind vane via `as5600` on the existing I²C bus. Calibration is the work: bearing friction must be low (proper miniature ball bearings, dry PTFE only), AS5600 magnet must be diametrically magnetized and well-aligned, m/s-per-Hz factor calibrated against a handheld anemometer or a known-speed drive. See "Future Expansion" section for the full hardware design, magnet-status logging, and HA integration (Beaufort scale, wind chill, mounting-height disclaimer).
12. **Stage 10 — Lower-priority sensors (later, as needed).** Soil temperature (DS18B20), soil moisture (capacitive ADC), UV (VEML6075/LTR390), LDR. All slot onto the existing I²C bus or free ADC pins, all gated on 3V3_C. Each is a YAML-component-add operation; no architectural changes. Defer until the base station + wind + lightning are deployed and stable.
